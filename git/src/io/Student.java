...
public class Student {
	...
	public static void setCivilStatus(String newValue) {
	...
	private static String StudentRecord() {
		return IDNumber + Delimeter + Name + Delimeter + Gender + Delimeter
				+ CivilStatus + Delimeter + Email;
	}
	...
	public static String GetNew_IDNumber() {
		String MyIDNum = "0";
		try {
			FileInputStream Files = new FileInputStream(DataFile);
	...
	private static void Save_TextFile(String vFileName, String newValue) {
		try {
			FileWriter writeFile = new FileWriter(vFileName, true);
	...
	public static void Search_IDNumber(String SearchKey) {
		isRecordFound = false;
		try {
			FileInputStream FileRec = new FileInputStream(DataFile);
			DataInputStream myInput = new DataInputStream(FileRec);
			while ((thisLine = myInput.readLine()) != null) {
				myIDNumber = thisLine.split("\\" + Delimeter);
				if (Integer.parseInt(myIDNumber[0]) == Integer
						.parseInt(SearchKey)) {
